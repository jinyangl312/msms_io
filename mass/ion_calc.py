...
def calc_b_ions(peptide, modinfo, xlmassinfo=None):
    ...
    if xlmassinfo == None:
        ...
        pepmass = b_ions[-1] + aamass.aa_mass_dict[peptide[-1]] + \
            modmass_list[len(peptide)] + \
            modmass_list[len(peptide)+1] + aamass.mass_H2O
    else:
        for i in range(len(peptide)-1):
            mass_nterm += aamass.aa_mass_dict[peptide[i]
                                              ] + modmass_list[i+1] + xlmassinfo[i+1]
            b_ions.append(mass_nterm)
        pepmass = b_ions[-1] + aamass.aa_mass_dict[peptide[-1]] + \
...
def calc_pepmass_aa_ini(peptide, modinfo=""):
    ...
    pepmass = mass_nterm + \
        aamass_aa_ini.aa_mass_dict[peptide[-1]] + aamass_aa_ini.mass_H2O
    return pepmass
...
def calc_ion_modloss(ions, peptide, modinfo, N_term=True):
    ...
    modmass_list, modloss_list, modname_list = calc_mod_mass_list(
        peptide, modinfo)
    ret = [0]*len(ions)
    if N_term:
